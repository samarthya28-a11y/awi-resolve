...
| deploy_software | **v1.1.** Install an approved product. Takes ONLY a `productId` from the compiled-in catalog (`agent/catalog.js`) — for catalog installs the AI can never supply a URL, filename or arguments. Each entry pins an HTTPS url + sha256 + fixed silent args + a verify path. Flow: download → **verify sha256 (abort on mismatch)** → run fixed args → verify installed. An entry without a valid pinned hash is refused (fail closed). |
| deploy_from_manual_url | **v1.2.** Install from a customer-attached setup/install manual when the product is not in the catalog. AI may pass `productName`, an HTTPS `.exe`/`.msi` `url` taken from the shared manual (or an explicit customer-typed link), and optional `sha256`. Agent enforces: HTTPS only (redirects must stay HTTPS), `.exe`/`.msi` only (scripts/archives refused), fixed silent args (`/S` or `msiexec /qn` — never model-supplied), optional checksum verify (abort on mismatch), size cap, temp cleanup, and a template consent prompt that shows the **full URL**. Still no free-form shell. Requires deployment licence capability. |
...
outside 4 named key paths · arbitrary installer arguments or non-HTTPS/script payloads ·
...
**Deployment (v1.1 / v1.2) — how it stays constrained.** Catalog installs (`deploy_software`)
remain fully pinned: the AI selects an entry by id, and the agent owns the URL, checksum,
and arguments. Shared-manual installs (`deploy_from_manual_url`, v1.2) deliberately allow
the AI to pass a URL from a customer-supplied guide, but the agent still refuses anything
that is not HTTPS `.exe`/`.msi`, never accepts model-supplied install arguments, verifies
sha256 when provided, and always requires customer Yes/No with the full URL visible.
Verified guards: off-catalog id refused for `deploy_software`; http/file/unc refused;
scripts/archives refused; non-HTTPS redirect refused; **checksum mismatch aborts before
execution** when a hash is supplied; consent prompts are template-generated by the agent.

**Timeout ordering (v1.1).** The cloud's wait for a tool result must always exceed the
...
Current: consent 60s < tool 120s; deployments 480s (consent + download + install).
