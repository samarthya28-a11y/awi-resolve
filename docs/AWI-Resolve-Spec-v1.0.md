...
| deploy_software | **v1.1.** Install an approved product. Takes ONLY a `productId` from the compiled-in catalog (`agent/catalog.js`) — the AI can never supply a URL, filename or arguments. Each entry pins an HTTPS url + sha256 + fixed silent args + a verify path. Flow: download → **verify sha256 (abort on mismatch)** → run fixed args → verify installed. An entry without a valid pinned hash is refused (fail closed). Non-catalog software falls back to Level-1 guidance. |
...
**Deployment (v1.1) — how it stays inside Tier-X.** "Installing software outside the
signed-installer list" remains forbidden. `deploy_software` satisfies this because the
installer list *is* `agent/catalog.js`, compiled into the agent: the AI selects an entry
by id, and the agent — not the model — owns the URL, the checksum, and the arguments.
Verified guards (tested): off-catalog id refused; a URL passed as an id refused;
model-supplied url/args ignored; missing/invalid pinned hash fails closed; non-HTTPS
refused; **checksum mismatch aborts before execution**; consent prompt is
template-generated by the agent.
