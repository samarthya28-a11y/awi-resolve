...
| deploy_software | **v1.1.** Install an Alpha Web globally approved product. Takes ONLY a `productId` from the compiled-in catalog (`agent/catalog.js`). Each entry pins an HTTPS url + sha256 + fixed silent args + a verify path. |
| deploy_org_software / deploy_pinned_software | **v1.3.** Install from the **customer IT-admin approved software library**. AI calls `deploy_org_software` with only a `productId` from `list_org_approved_software`. The orchestrator resolves the admin-pinned HTTPS URL + required sha256 and forwards `deploy_pinned_software` to the agent. Agent enforces HTTPS, `.exe`/`.msi`, fixed silent args, checksum verify, and Yes/No consent showing the full URL. Manuals + links only are stored in the library (no installer binaries hosted). End-user paperclip attachments cannot unlock installs. |
| run_powershell | **v1.4 Full IT Support.** Consented arbitrary PowerShell on the end-user PC. Dual-gated: licence plan must be `full` (`caps.fullSupport`) **and** the customer IT admin must enable `allowFullItSupport` for that org. Consent prompt is template-generated and always shows the **exact command**. Caps: command ≤4000 chars, ~90s runtime, stdout/stderr truncated (~32 KB). Prefer allowlisted tools when they suffice. AI must refuse illegitimate asks (hacking others, attacking other systems, fraud, weakening protection to hide malware, exfiltrating personal data). |

**Tier-X — structurally impossible (not implemented in the agent binary)**

Free-form shell/PowerShell execution **except** the dual-gated `run_powershell` tool under Full IT Support (v1.4) · reading/writing user documents · browser data,
credential stores, password ops · disabling AV/firewall/Defender · registry writes
outside named paths (except via consented Full IT Support PowerShell) · arbitrary installer arguments, non-HTTPS/script payloads, or
installs from end-user-supplied URLs · outbound uploads of any file content other than the
fixed log paths · anything touching other machines on the network (also refused as illegitimate under Full mode).

Adding a tool = code change + spec update + red-team review. Never config-only.

**Full IT Support (v1.4).** Plan `full` includes Pro capabilities plus `run_powershell`. The tool is
hidden from the AI tool list unless both gates pass for the session. Orchestrator still rejects
`run_powershell` if either gate fails (defence in depth). Org admins toggle the flag on
`/admin/software` (Approved Software Library page).

**Deployment (v1.1 / v1.3) — how it stays constrained.** Catalog installs (`deploy_software`)
remain fully pinned in `agent/catalog.js`. Org installs (v1.3) are pinned by each customer's
IT admin in the org software library (manual text + HTTPS link + sha256). The AI never
supplies a download URL — only a library `productId`. Free-form `deploy_from_manual_url`
(v1.2 experiment) is removed. Verified guards: http/file/unc refused; scripts/archives
refused; non-HTTPS redirect refused; **checksum mismatch aborts before execution**; consent
prompts are template-generated by the agent.

**Timeout ordering (v1.1 / v1.4).** The cloud's wait for a tool result must always exceed the
agent's consent window, or the cloud could abandon a Tier-2 action while the customer
is still deciding — and the action could then execute after the AI concluded it hadn't.
Current: consent 60s < tool 120s; deployments 480s; Full IT Support PowerShell 180s
(consent + up to 90s run + buffer).
